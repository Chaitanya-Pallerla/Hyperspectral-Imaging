% =========================================================================
% microbial_extraction_VIS_400_1000nm.m
%
% Description:
%   Batch pipeline for per-colony hyperspectral spectral extraction
%   from microbial agar plate images — VIS range (400–1000 nm, 448 bands).
%
...
%   area filtering, calibrates the VIS cube, extracts and SG-smooths
...
function microbial_extraction_VIS_400_1000nm()
...
% Root folder containing all Yang_* dataset folders (VIS 400-1000nm data)
% Example: "C:\Users\YourName\Data\Yang_400_1000\"
root_path = 'F:\Yang_Extra\Extra_400-1000';   % <-- CHANGE THIS

% Root folder where all processed outputs will be saved
out_root  = fullfile(root_path, 'Processed_400_1000nm');  % <-- CHANGE IF NEEDED
...
for k = 1:numel(dsets)
    ...
    plot(raw_info.Wavelength, dark_avg1, 'r', 'LineWidth', 1.5);
    ...
    fig = figure('Visible','off');
    plot(raw_info.Wavelength, white_avg1, 'b', 'LineWidth', 1.5);
    ...
    % =====================================================================
    %  STEP 3: CROP TO 400–1000 nm AND RESAMPLE TO 448 BANDS
    % =====================================================================

    wl      = raw_info.Wavelength(:);
    wl_mask = wl >= 400 & wl <= 1000;
    ...
    if numel(idx) == TARGET_BANDS
        ...
        if ~any(wl_mask)
            warning('No wavelengths within 400-1000 nm for %s — skipping.', dataset_name);
            continue;
        end
        wl_sel   = linspace(max(400, min(wl(wl_mask))), ...
                            min(1000, max(wl(wl_mask))), TARGET_BANDS).';
    ...
    writematrix(wl_sel, fullfile(data_dir, ...
        sprintf('wavelengths_%s_400_1000nm_448b.csv', dataset_name)));
    save(fullfile(data_dir, ...
        sprintf('wavelengths_%s_400_1000nm_448b.mat', dataset_name)), 'wl_sel');
    ...
    for i = 1:numColonies
        colony_mask        = zeros(size(BW));
        colony_mask(CC.PixelIdxList{i}) = 1;

        imwrite(uint8(colony_mask)*255, ...
            fullfile(bw_dir, sprintf('BW_colony_%03d.png', i)));

        % Band-by-band extraction (matches single-sample pipeline)
        extracted_data = zeros(size(cube_sel,3), sum(colony_mask(:) > 0));
        mask_vec       = logical(colony_mask(:));

        for j = 1:size(cube_sel, 3)
            band_img = cube_sel(:,:,j) .* colony_mask;
            band_vec = band_img(:);
            extracted_data(j,:) = band_vec(mask_vec);
        end

        E_col   = mean(extracted_data, 2);
        E_col_f = sgolayfilt(E_col, SG_ORDER, SG_WINDOW);
        stacked_data = [stacked_data, E_col_f];  %#ok

        raw_var  = sprintf('E_%s_colony_%d_400_1000nm_448b', dataset_name, i);
    ...
    csv_file = fullfile(data_dir, sprintf('stacked_%s_400_1000nm_448b.csv', dataset_name));
    mat_file = fullfile(data_dir, sprintf('stacked_%s_400_1000nm_448b.mat', dataset_name));
    writematrix(stacked_data.', csv_file);
    save(mat_file, 'stacked_data', 'wl_sel');

    copyfile(csv_file, fullfile(stack_dir, sprintf('stacked_%s_400_1000nm_448b.csv', dataset_name)));
    copyfile(mat_file, fullfile(stack_dir, sprintf('stacked_%s_400_1000nm_448b.mat', dataset_name)));

    fig = figure('Visible','off');
    plot(wl_sel, stacked_data, 'LineWidth', 1.0);
    title(sprintf('Stacked Spectra — %s (400-1000 nm)', dataset_name), 'Interpreter','none');
    xlabel('Wavelength (nm)'); ylabel('Reflectance');
    saveas(fig, fullfile(stack_dir, sprintf('stacked_%s_400_1000nm_448b.png', dataset_name)));
    close(fig);
